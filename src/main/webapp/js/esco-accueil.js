...
( function($) {
	...
	function mouseenter(){
		$('#navigation').find('li').mouseenter(function(e) {
			...
			//console.log(($(this).outerHeight()-$(this).find('img').outerHeight())/2+"px");
			//$(this).find('img').css('margin-top',($(this).outerHeight()-$(this).find('img').outerHeight())/2+"px");
			var image_courante = $('#slider').find('img:visible').css('height',slider_img.outerHeight()).css('width',slider_img.outerWidth());
			var image_suivante = $('#slider').find('li').eq(pos_suivant).children('img').css('height',slider_img.outerHeight()).css('width',slider_img.outerWidth());
		...
		$('#content img').each(function() {
			$(this).load(function(e) {
				...
				if (nb_img_loaded == nb_img) {
					...
					totalWidth = slider_img.outerWidth()+nav_img.outerWidth()+$('.meta').outerWidth()
					totalHeight = slider_img.outerHeight();
	...
	function doresize(){
		...
		if ((resizeWidth + 50) > $(window).width()) {
			...
			nav_width = totalWidth-slider_img.outerWidth();
		}
		//nav_width = totalWidth-slider_img.outerWidth();
		...
		$('#slider').css('width',slider_img.outerWidth());
		...
		list_li.each(function(){
			...
			meta.css('margin-top',(li_height-meta.outerHeight())/2+"px");
	...
	function clicknext(){
		$('.boutons').find('a.next').click(function(e) {
			...
			var image_courante = $('#slider').find('img:visible').css('height',slider_img.outerHeight()).css('width',slider_img.outerWidth());
			var image_suivante = $('#slider').find('li').eq(li_suivant.index()).children('img').css('height',slider_img.outerHeight()).css('width',slider_img.outerWidth());
	...
	function clickprev(){
		$('.boutons').find('a.prev').click(function(e) {
			...
			var image_courante = $('#slider').find('img:visible').css('height',slider_img.outerHeight()).css('width',slider_img.outerWidth());
			var image_precedente = $('#slider').find('li').eq(li_precedent.index()).children('img').css('height',slider_img.outerHeight()).css('width',slider_img.outerWidth());
